<?php
    ...
    function renameTheFile($file_id,$new_file_name,$file_path){//$operation == 0
        ...
        if(file_exists($file_path)) {
            $new_file_path = $upload_file_folder_path.$_SESSION[$tb_account]."_".$new_file_name;
            if(rename($file_path,$new_file_path)){
                require("connect_to_mysql.php");
                $sql_update = "UPDATE members_files
                        SET $tb_file_name='$new_file_name'
                        WHERE $tb_file_id='$file_id' ";
                echo $sql_update."<br>";
    ...
    function deleteTheFile($file_id,$file_path){//$operation == 1
        ...
        if(file_exists($file_path)) {
            if(unlink($file_path)){
                require("connect_to_mysql.php");
                $sql_delete = "DELETE FROM members_files WHERE $tb_file_id = '$file_id'";
